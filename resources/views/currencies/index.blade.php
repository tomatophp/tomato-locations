<x-tomato-admin-layout>
    ...
    <div class="pb-12" v-cloak>
        <div class="mx-auto">
            @if(\TomatoPHP\TomatoLocations\Models\Currency::count())
            <x-splade-table :for="$table" striped>
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <Link href="/admin/currencies/{{ $item->id }}" class="px-2 text-blue-500" slideover>
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-eye class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                                <span>{{trans('tomato-admin::global.crud.view')}}</span>
                            </div>
                        </Link>
                        <Link href="/admin/currencies/{{ $item->id }}/edit" class="px-2 text-yellow-400" slideover>
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-pencil class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                                <span>{{trans('tomato-admin::global.crud.edit')}}</span>
                            </div>
                        </Link>
                        <Link href="/admin/currencies/{{ $item->id }}"
